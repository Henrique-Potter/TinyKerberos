...
from coap_client_calls import sec_client_auth_put_call, sec_register_device_put_call, register_device_put_call, \
    sec_client_discover_device_put_call, client_discover_device_put_call, sec_client_device_interaction_put_call, \
    client_device_interaction_put_call
...
def benchmark_protocol(iterations, stop_between):
    ...
    if stop_between:
        input("About to start sec device interaction")
    benchmark_sec_client_device_interaction_put_call(2,iterations,measured_time,url, device_session_key, rad_ticket, owner_ticket)

    if stop_between:
        input("About to start non-sec client device discover")
    url = benchmark_client_discover_device_put_call(3, iterations, measured_time)

    if stop_between:
        input("About to start non-sec device interaction")
    benchmark_client_device_interaction_put_call(4, iterations, measured_time, url)

    if stop_between:
        input("About to start sec register device")
    benchmark_sec_register_device(5, iterations, measured_time)

    if stop_between:
        input("About to start non-sec register device")
    benchmark_register_device(6, iterations, measured_time)
...
def benchmark_sec_client_device_interaction_put_call(column, iterations, measured_time, url, device_session_key, rad_ticket, owner_ticket):
...
def benchmark_client_device_interaction_put_call(column, iterations, measured_time, url):
